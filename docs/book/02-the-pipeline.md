...
`src/cli.ts` (Commander) exposes four top-level commands (`feature` has subcommands):
...
2. .claude/agents/<name>.md   × N     ← one per prompts/agents/*: frontmatter
                                         (name/description/tools [+ per-agent hooks])
                                         + rendered prompt body
3. .claude/skills/<name>/SKILL.md × N ← one per prompts/skills/*: frontmatter
...
```
# <title>                (H1 + a "Generated by ai-factory … do not hand-edit" banner)
