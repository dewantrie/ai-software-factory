...
export const claudeCode: PlatformAdapter = {
  ...
  async generate({ targetRoot, manifest, agents, skills, profileBody }) {
    ...
    // 5. FACTORY.md — the human-facing explainer. `.claude/` now holds hooks,
    //    permission rules and possibly a sandbox; without this, someone opening
    //    an installed repo has no way to learn where any of it came from.
    const factoryPath = join(targetRoot, ".claude", "FACTORY.md");
    writeFile(factoryPath, buildFactoryDoc(manifest, agents.length, skills.length));
...
function buildFactoryDoc(manifest: Manifest, numAgents: number, numSkills: number): string {
  ...
  const scopedAgents = Object.keys(agentAllowMap(manifest));
  const stop = manifest.hooks?.stopOnFailingValidation === true;
  const capture = manifest.hooks?.captureAgentOutput === true;
  const sandbox = manifest.sandbox === true;

  const row = (on: boolean, what: string, why: string, off: string) =>
    on ? `| \`${what}\` | ${why} | ${off} |` : null;

  const settingsRows = [
    row(
      forbidden.length > 0,
      "hooks.PreToolUse",
      "Runs the path guard on every edit, blocking the `forbidden:` list session-wide.",
      "remove `paths.forbidden` from `.factory.yaml`",
    ),
    row(
      forbidden.length > 0,
      "permissions.deny",
      "`Edit(<glob>)` per forbidden glob. Also covers the file commands Claude Code recognises inside Bash (`tee`, `sed`, `> file`).",
      "remove `paths.forbidden`",
    ),
    row(
      sandbox,
      "sandbox",
      "OS-level Bash sandbox, plus `filesystem.denyWrite` per forbidden glob — the only layer that also stops a subprocess the agent spawns.",
      "set `sandbox: false` (or remove the key)",
    ),
    row(
      stop,
      "hooks.Stop",
      "Refuses to end a turn while `" + manifest.commands.test + "` fails and the tree is dirty.",
      "set `hooks.stop-on-failing-validation: false`",
    ),
    row(
      capture,
      "hooks.SubagentStop",
      "Records each agent's final output to `.factory/runs/<session>/`.",
      "set `hooks.capture-agent-output: false`",
    ),
  ].filter((r): r is string => r !== null);

  return `# AI Factory for Claude Code
...
- \`.claude/agents/<name>.md\` — ${numAgents} subagents. Each runs in its own context with its own tools; editing agents carry a per-agent path-guard hook.
- \`.claude/skills/<name>/SKILL.md\` — ${numSkills} orchestrators. Invoke with \`/<name>\`, or let Claude match your request against the skill's description.
...
Tier 3 runs the full ${numAgents}-agent chain with three approval checkpoints (story,
...
|---|---|---|
${settingsRows.join("\n")}`
    : "_This manifest declares no scope and no optional hooks, so the factory adds nothing to settings.json._"
}

## Path scoping — ${scopedAgents.length > 0 ? "enforced" : "prompt-only"}

${
  scopedAgents.length > 0
    ? `These agents may only edit their declared paths, enforced by a \`PreToolUse\` hook
in each one's frontmatter: **${scopedAgents.join("**, **")}**. An agent with no list
...
\`.claude/hooks/factory-scope.json\`.`
    : `No per-agent allow-lists are declared in \`.factory.yaml\`, so agents are scoped by
prompt only. Add \`paths.backend\`, \`paths.frontend\`, etc. to enforce them.`
}

**Know the boundary.** The hook fires on \`Write\`/\`Edit\`/\`MultiEdit\`/\`NotebookEdit\`.
A builder also has \`Bash\`, and a shell write never reaches it${forbidden.length > 0 ? " — the `permissions.deny` rules catch the file commands Claude Code recognises, and `sandbox: true` catches subprocesses too" : ""}.
